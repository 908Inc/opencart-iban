...
class ControllerExtensionPaymentOpencartIban extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('opencart_iban', $this->request->post);
		...
		$data['opencart_iban_iban'] = isset($this->request->post['opencart_iban_iban']) ? $this->request->post['opencart_iban_iban'] : $this->config->get('opencart_iban_iban');
		$data['opencart_iban_code'] = isset($this->request->post['opencart_iban_code']) ? $this->request->post['opencart_iban_code'] : $this->config->get('opencart_iban_code');
		$data['opencart_iban_client_key'] = isset($this->request->post['opencart_iban_client_key']) ? $this->request->post['opencart_iban_client_key'] : $this->config->get('opencart_iban_client_key');
		$data['opencart_iban_client_name'] = isset($this->request->post['opencart_iban_client_name']) ? $this->request->post['opencart_iban_client_name'] : $this->config->get('opencart_iban_client_name');

		// Purpose templates (per language)
		$this->load->model('localisation/language');

		$data['languages'] = $this->model_localisation_language->getLanguages();
		$data['opencart_iban_purpose'] = array();

		foreach ($data['languages'] as $language) {
			$language_id = (int)$language['language_id'];
			$key = 'opencart_iban_purpose_' . $language_id;
			...
			$data['opencart_iban_purpose'][$language_id] = $value;
		}
		...
		// Autoclient
		if (isset($this->request->post['opencart_iban_autoclient'])) {
			$data['opencart_iban_autoclient'] = $this->request->post['opencart_iban_autoclient'];
		} else {
			$data['opencart_iban_autoclient'] = $this->config->get('opencart_iban_autoclient');
		}

		if (isset($this->request->post['opencart_iban_paid_order_status_id'])) {
			$data['opencart_iban_paid_order_status_id'] = $this->request->post['opencart_iban_paid_order_status_id'];
		} else {
			$data['opencart_iban_paid_order_status_id'] = (int)($this->config->get('opencart_iban_paid_order_status_id') ?: 5);
		}
		...
		// Sort Order
		if (isset($this->request->post['opencart_iban_sort_order'])) {
			$data['opencart_iban_sort_order'] = $this->request->post['opencart_iban_sort_order'];
		} else {
			$data['opencart_iban_sort_order'] = $this->config->get('opencart_iban_sort_order');
		}
	...
	protected function validate() {
		...
		if (!isset($this->request->post['opencart_iban_iban']) || trim($this->request->post['opencart_iban_iban']) === '') {
			$this->error['iban'] = $this->language->get('error_iban');
		} else {
			$iban = strtoupper(preg_replace('/\\s+/', '', (string)$this->request->post['opencart_iban_iban']));
			$this->request->post['opencart_iban_iban'] = $iban;
		...
		if (!isset($this->request->post['opencart_iban_code']) || trim($this->request->post['opencart_iban_code']) === '') {
			$this->error['code'] = $this->language->get('error_code');
		} else {
			$code = preg_replace('/\\s+/', '', (string)$this->request->post['opencart_iban_code']);
			$this->request->post['opencart_iban_code'] = $code;
		...
		if (!isset($this->request->post['opencart_iban_client_key']) || trim($this->request->post['opencart_iban_client_key']) === '') {
			$this->error['client_key'] = $this->language->get('error_client_key');
		} else {
			$this->request->post['opencart_iban_client_key'] = trim((string)$this->request->post['opencart_iban_client_key']);
		}

		if (!isset($this->request->post['opencart_iban_client_name']) || trim($this->request->post['opencart_iban_client_name']) === '') {
			$this->error['client_name'] = $this->language->get('error_client_name');
		} else {
			$this->request->post['opencart_iban_client_name'] = trim((string)$this->request->post['opencart_iban_client_name']);
		}
